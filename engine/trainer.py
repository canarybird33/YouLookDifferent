...
def do_train(
    ...
    elif cfg.MODEL.Evaluate == "skip":
        pass
    else:
        raise ValueError('Only support pretrain_choice for \'ClothChangingSetting\' and \'StandardSetting\', but got {}'.format(cfg.MODEL.Evaluate))
    ...
    def log_validation_results(engine):
        if engine.state.epoch % eval_period == 0:
            if cfg.MODEL.Evaluate == "both":
                ...
                logger.info("Here Are the Validation Results - Epoch: {}".format(engine.state.epoch))
                logger.info("mAP: {:.1%}".format(mAP))
                for r in [1, 5, 10, 20, 50]:
                    logger.info("CMC curve, Rank-{:<3}:{:.1%}".format(r, cmc[r - 1]))
                ...
                ValueError("set 'cfg.MODEL.Evaluate' appropriately")
