...
if __name__ == '__main__':
    ...
    if one_time_test:
        ...
        for i in range (0, 10, 1):
            ...
            Total_CC_cmc_.append(100*np.array(CC_cmc_))
            Total_CC_mAP_.append(100*np.array(CC_mAP_))
            Total_SS_cmc_.append(100*np.array(SS_cmc_))
            Total_SS_mAP_.append(100*np.array(SS_mAP_))

        CC_cmc = sum(Total_CC_cmc_) / len(Total_CC_cmc_)
        CC_mAP = sum(Total_CC_mAP_) / len(Total_CC_mAP_)
        SS_cmc = sum(Total_SS_cmc_) / len(Total_SS_cmc_)
        SS_mAP = sum(Total_SS_mAP_) / len(Total_SS_mAP_)

        var_CC_cmc = sum((x-CC_cmc)**2 for x in Total_CC_cmc_) / (len(Total_CC_cmc_)-1)
        var_CC_mAP = sum((x-CC_mAP)**2 for x in Total_CC_mAP_) / (len(Total_CC_mAP_)-1)
        var_SS_cmc = sum((x-SS_cmc)**2 for x in Total_SS_cmc_) / (len(Total_SS_cmc_)-1)
        var_SS_mAP = sum((x-SS_mAP)**2 for x in Total_SS_mAP_) / (len(Total_SS_mAP_)-1)
