...
def get_mask_boarder(orig_mask, display=False):
    ...
    eroded_mask = mask_dilation_erosion(orig_mask, 'erode', proportion = 0.033, iter = 2)
...
def do_inpainting(img, mask, display=False):
    radius = int(max(np.array(orig_mask.shape))*0.1)
...
def make_same_sizes (img1, img2, img2_mask):
    ...
    if h1>h2: # increase the height of img2
        ...
        img2_m_p = cv2.copyMakeBorder(img2_mask, int((h1 - h2) / 2), int((h1 - h2) / 2), 0, 0, borderType=cv2.BORDER_REFLECT)
        img1_p = np.copy(img1)
...
def do_deformed_mesh(background, img, mask, display=False):
    ...
    extended_body_area_mask = cv2.copyMakeBorder(gray_mask, dh, dh, dw, dw, borderType=cv2.BORDER_CONSTANT, value=(0, 0, 0))
    ...
    deformed_image = deformed_extended_img[y:y+h, x:x+w, :] # rectangular contour of image ==> should be resized to original image size
    deformed_mask = deformed_extended_msk[y:y+h, x:x+w] # rectangular contour of mask ==> should be resized to original image size
...
def do_scratch(img, min_prop, max_prop, seed, display= False):
    ...
    if p1>p2:
        ...
        resized_img = cv2.copyMakeBorder(resized_img, int(add_to_length/2), int(add_to_length/2), 0, 0, cv2.BORDER_CONSTANT, (0,0,0))
        ...
        resized_img = cv2.copyMakeBorder(resized_img, 0, 0, int(add_to_width/2), int(add_to_width/2), cv2.BORDER_CONSTANT, (0,0,0))
        resized_img = cv2.resize(resized_img, (w1, h1))
        assert img.shape == resized_img.shape
...
def write_img(dir, img_array, img_name):
...
def transform_one_image():
    source_dir = "/home/socialab157/Desktop/YLD_fig"
    dir_to_save_noneID = "/home/socialab157/Desktop/YLD_fig/process"

    img1 = "orig_11_21_0.jpg"
    img2 = "orig_11_20_0.jpg"
    maks1 = "mask_11_21_0.jpg"
    maks2 = "mask_11_20_0.jpg"
    key1 = "11_21_0.json"
    key2 = "11_20_0.json"
    ...
    for i in range(1, 20, 1):
        dir = os.path.join(dir_to_save_noneID, str(i))
        os.makedirs(dir, exist_ok=True)

        img_arry = load_img(source_dir, img_name)
        orig_mask = load_mask(source_dir, mask_name)
        keypoints = load_keypoints(source_dir, key1, False)

        background = HiFill_inpainting.HiFill (img_arry, orig_mask, False)
        write_img(dir, background, "{}_background.jpg".format(img_name))

        head_mask = get_head_mask(orig_mask, keypoints, False)
        write_img(dir, head_mask, "{}_head_mask.jpg".format(img_name))

        mask_boarder, dilated_mask, eroded_mask = get_mask_boarder(orig_mask, False)
        write_img(dir, mask_boarder, "{}_boarder_area.jpg".format(img_name))
        write_img(dir, dilated_mask, "{}_dilated_mask.jpg".format(img_name))
        write_img(dir, eroded_mask, "{}_eroded_mask.jpg".format(img_name))

        final_mask = get_final_mask(head_mask, mask_boarder, False)
        write_img(dir, final_mask, "{}_final_mask.jpg".format(img_name))

        HiFill_head_border = HiFill_inpainting.HiFill(img_arry, final_mask, False)
        write_img(dir, HiFill_head_border, "{}_boarder_head_inpainted.jpg".format(img_name))
        ...
        write_img(dir, imploded_img, "{}_imploded_img.jpg".format(img_name))
        write_img(dir, imploded_mask, "{}_imploded_mask.jpg".format(img_name))

        img_deformed_scratched, mask_deformed_scratched, img_deformed_NOscratched, mask_deformed_NOscratched = do_deformed_mesh(background= background, img = imploded_img, mask = imploded_mask, display= False)  # can be done on mask and img
        write_img(dir, img_deformed_scratched, "{}_img_deformed_scratched.jpg".format(img_name))
        write_img(dir, mask_deformed_scratched, "{}_mask_deformed_scratched.jpg".format(img_name))
        write_img(dir, img_deformed_NOscratched, "{}_img_deformed_NOscratched.jpg".format(img_name))
        write_img(dir, mask_deformed_NOscratched, "{}_mask_deformed_NOscratched.jpg".format(img_name))
...
if __name__ == '__main__':
    ...
    dir_to_save_noneID = "/media/socialab157/2cbae9f1-6394-4fa9-b963-5ef890eee044/B_DATASETS/Long_term_datasets/LTCC_ReID/LTCC_nonID"
    ...
    if one_sub_folder:
        ...
        for F_index, fld_name in enumerate(folder_names):
            ...
            for index, img_name in enumerate(images_names):
                ...
                keypoints = load_keypoints(os.path.join(json_files_dir, fld_name), img_name, False)
        ...
        for p_index, p_fld_name in enumerate(parent_fld):
            ...
            for F_index, fld_name in enumerate(folder_names):
                ...
                for index, img_name in enumerate(images_names):
                    ...
                    keypoints = load_keypoints(os.path.join(json_files_dir, p_fld_name, fld_name), img_name, False)
                    if keypoints is None or orig_mask is None or img_arry is None:
                        print("(img/mask/keypoint)data did not find: {}".format(img_name))
                        continue
